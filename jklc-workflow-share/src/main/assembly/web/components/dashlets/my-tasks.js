...
(function()
{
   ...
   YAHOO.lang.augmentObject(Alfresco.dashlet.MyTasks.prototype,
   {
      ...
      renderCellTaskInfo: function MyTasks_onReady_renderCellTaskInfo(elCell, oRecord, oColumn, oData)
      {
         ...
         {
        	 var packageNodeRef= data.workflowInstance["package"];
        		  ...
        		  successCallback : {
        		    fn : function (res) {
        		      ...
        		      if(result.children.length>0){
        		    	  ...
        		    	  this.callFunction(elCell, oRecord, oColumn, oData,data,docNodeRef);
        		      }else{
        		    	  this.callFunction(elCell, oRecord, oColumn, oData,data,"");
        		      }
        		    },
        		    scope : this
        		  },
      ...
      callFunction:function callFunction(elCell, oRecord, oColumn, oData,data,docNodeRef)
      {
       ...
       if (data.isEditable)
       {
          ...
          if(docNodeRef){
        	  href = $siteURL('task-edit?taskId=' + taskId +"&nodeRef="+docNodeRef) + '" class="theme-color-1" title="' + this.msg("title.editTask");
          }
       }
       else
       {
         ...
         else
         {
            if (data.isEditable)
            {
            	...
            	var workflowInstance = oRecord.getData("workflowInstance");
           	    var packageNodeRef= workflowInstance["package"];
              	 Alfresco.util.Ajax.jsonGet({
              		  url:Alfresco.constants.PROXY_URI + "slingshot/node/" +packageNodeRef.replace("://", "/"),
              		  successCallback : {
              		    fn : function (res) {
              		      ...
              		      if(result.children.length>0){
              		    	  var docNodeRef=result.children[0].nodeRef;
              		    	 desc += '<a href="' + $siteURL('task-edit?taskId=' + data.id + "&nodeRef="+docNodeRef) + '" class="edit-task" title="' + this.msg("title.editTask") + '">&nbsp;</a>';
              		      }else{
